...
proportions=read.csv("C:/Users/Sara/Documents/HHMI_projects/Meghan_Garrett/2018_10_05/proportions.csv", header=T, stringsAsFactors = F)
...
colnames(propnd)
# [1] "id"                   "Virus_Protein_Loc_AA" "Oligo"                "X006.1"               "X006.2"               "X016.1"              
# [7] "X016.2"               "X067.1"               "X067.2"               "X072.1"               "X072.2"               "X10E8.1"             
# [13] "X10E8.2"              "X2F5.1"               "X2F5.2"               "X4E10.1"              "X4E10.2"              "Beadsonly.Lib1.1"    
# [19] "Beadsonly.Lib1.2"     "Beadsonly.Lib2.1"     "Beadsonly.Lib2.2"     "Beadsonly.Lib2a"      "Beadsonly.Lib2b"      "Input.Lib1"          
# [25] "Input.Lib2a"          "Input.Lib2b"          "Input.Lib2"           "mean_beads"           "Virus"                "Protein"             
# [31] "Locus"     

#need to look just at the actual samples, not input or metadata

samples=colnames(propnd[c(4:17)])
  ...
  for (j in 1:length(prot)){
    ...
    for (m in 1:length(samples)){
      ...
      for (k in 1:length(sorted$Locus)){
        ...
        locustotal[k]=(sum(sorted[overlap,samples[m]])-sum(sorted[overlap, "mean_beads"]))/sum(sorted[overlap,"Input.Lib1"])
      }
